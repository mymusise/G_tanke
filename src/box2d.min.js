...
b2Settings.b2Assert = function(b) {
	if (!b) {
		var c;
		c.x++
	}
...
var b2BroadPhase = function(f, g) {
	...
	var e;
	for (d = 0; d < b2Settings.b2_maxProxies - 1; ++d) {
		e = new b2Proxy();
...
b2BroadPhase.prototype = {
	...
	DestroyProxy: function(s) {
		...
		for (var p = 0; p < this.m_queryResultCount; ++p) {
	...
	QueryAABB: function(f, b, c) {
		var j = new Array();
		var g = new Array();
		this.ComputeBounds(j, g, f);
		var a = 0;
		var k = 0;
		var d = [a];
		var m = [k];
		this.Query(d, m, j[0], g[0], this.m_bounds[0], 2 * this.m_proxyCount, 0);
		this.Query(d, m, j[1], g[1], this.m_bounds[1], 2 * this.m_proxyCount, 1);
		var h = 0;
		for (var e = 0; e < this.m_queryResultCount && h < c; ++e, ++h) {
			var l = this.m_proxyPool[this.m_queryResults[e]];
			b[e] = l.userData
		}
		this.m_queryResultCount = 0;
		this.IncrementTimeStamp();
...
b2Collision.b2CollidePolyAndCircle = function(x, m, k, C) {
	...
	var g = k.m_radius;
	for (var A = 0; A < m.m_vertexCount; ++A) {
...
b2Distance.ProcessTwo = function(j, h, b, k, i) {
	var f = -i[1].x;
	var e = -i[1].y;
	var d = i[0].x - i[1].x;
	var c = i[0].y - i[1].y;
...
b2Distance.ProcessThree = function(q, D, m, C, G) {
	var k = G[0].x;
	var h = G[0].y;
	var z = G[1].x;
	var s = G[1].y;
...
b2Distance.InPoinsts = function(a, c, d) {
	for (var b = 0; b < d; ++b) {
...
b2Distance.Distance = function(h, n, c, a) {
	var d = new Array(3);
	var l = new Array(3);
	var s = new Array(3);
	var u = 0;
...
var b2PolyShape = function(s, o, a) {
	...
	if (s.type == b2Shape.e_boxShape) {
		...
		for (w = 0; w < this.m_vertexCount; ++w) {
...
b2Island.prototype = {
	...
	Solve: function(c, l) {
		...
		d.PreSolve();
		for (f = 0; f < this.m_jointCount;
			++f) {
			this.m_joints[f].PrepareVelocitySolver()
		}
		for (f = 0; f < c.iterations; ++f) {
			d.SolveVelocityConstraints();
			for (var e = 0; e < this.m_jointCount; ++e) {
		...
		if (b2World.s_enablePositionCorrection) {
			for (b2Island.m_positionIterationCount = 0; b2Island.m_positionIterationCount < c.iterations; ++b2Island.m_positionIterationCount) {
...
var b2ContactSolver = function(o, p, l) {
	...
	for (U = 0; U < this.m_constraintCount; U++) {
...
b2ContactSolver.prototype = {
	PreSolve: function() {
		...
		var A;
		var r;
		for (var w = 0; w < this.m_constraintCount; ++w) {
			var B = this.m_constraints[w];
			var o = B.body1;
			var n = B.body2;
			var t = o.m_invMass;
			var m = o.m_invI;
			...
			if (b2World.s_enableWarmStarting) {
				...
				for (v = 0; v < x; ++v) {
					...
					r = o.m_R;
					b = a.localAnchor1;
					var z = r.col1.x * b.x + r.col2.x * b.y;
					var y = r.col1.y * b.x + r.col2.y * b.y;
					r = n.m_R;
					b = a.localAnchor2;
					var g = r.col1.x * b.x + r.col2.x * b.y;
					var f = r.col1.y * b.x + r.col2.y * b.y;
					o.m_angularVelocity -= m * (z * p - y * q);
	...
	SolveVelocityConstraints: function() {
		...
		for (var E = 0; E < this.m_constraintCount; ++E) {
			...
			for (D = 0; D < G; ++D) {
				...
				s = p.m_R;
				b = a.localAnchor1;
				I = s.col1.x * b.x + s.col2.x * b.y;
				H = s.col1.y * b.x + s.col2.y * b.y;
				s = o.m_R;
				b = a.localAnchor2;
				...
				K = b2Math.b2Max(a.normalImpulse + x, 0);
				x = K - a.normalImpulse;
				r = x * e;
				q = x * d;
				f.x -= w * r;
				...
				var C = A * l + z * h;
				x = a.tangentMass * (-C);
				var B = J.friction * a.normalImpulse;
				K = b2Math.b2Clamp(a.tangentImpulse + x, -B, B);
				x = K - a.tangentImpulse;
				r = x * l;
				q = x * h;
				f.x -= w * r;
				f.y -= w * q;
	...
	SolvePositionConstraints: function(w) {
		var x = 0;
		var H;
		var y;
		for (var N = 0; N < this.m_constraintCount; ++N) {
			var Q = this.m_constraints[N];
			var s = Q.body1;
			var r = Q.body2;
			var f = s.m_position;
			var a = s.m_rotation;
			...
			for (var M = 0; M < q; ++M) {
				...
				H = s.m_R;
				y = b.localAnchor1;
				var L = H.col1.x * y.x + H.col2.x * y.y;
				var K = H.col1.y * y.x + H.col2.y * y.y;
				H = r.m_R;
				y = b.localAnchor2;
				var v = H.col1.x * y.x + H.col2.x * y.y;
				var u = H.col1.y * y.x + H.col2.y * y.y;
				var e = f.x + L;
				var d = f.y + K;
				var F = m.x + v;
				var D = m.y + u;
				var P = F - e;
				var O = D - d;
				var k = (P * A + O * z) + b.separation;
				x = b2Math.b2Min(x, k);
				var h = w * b2Math.b2Clamp(k + b2Settings.b2_linearSlop, -b2Settings.b2_maxLinearCorrection, 0);
				var t = -b.normalMass * h;
				var G = b.positionImpulse;
				b.positionImpulse = b2Math.b2Max(G + t, 0);
				t = b.positionImpulse - G;
				var n = t * A;
				var l = t * z;
				f.x -= E * n;
				f.y -= E * l;
		...
		return x >= -b2Settings.b2_linearSlop
	},
	PostSolve: function() {
		for (var d = 0; d < this.m_constraintCount;
			++d) {
			var g = this.m_constraints[d];
			var a = g.manifold;
			for (var b = 0; b < g.pointCount; ++b) {
				var f = a.points[b];
				var e = g.points[b];
...
Object.extend(b2PolyContact.prototype, {
	...
	Evaluate: function() {
		...
		if (a.pointCount > 0) {
			var h = [false, false];
			for (var f = 0; f < a.pointCount; ++f) {
				var m = a.points[f];
				m.normalImpulse = 0;
				m.tangentImpulse = 0;
				var l = m.id.key;
				for (var e = 0; e < this.m0.pointCount; ++e) {
					if (h[e] == true) {
						continue
					}
...
b2World.prototype = {
	...
	CreateJoint: function(e) {
		var c = b2Joint.Create(e, this.m_blockAllocator);
		c.m_prev = null;
	...
	QueryPoint: function(d, b) {
		var a, c;
		for (a = this.m_bodyList; a; a = a.m_next) {
			for (c = a.GetShapeList(); c != null; c = c.GetNext()) {
				if (c.TestPoint(b)) {
					d(a)
				}
			}
	...
	DebugDraw: function() {
		...
		for (b = world.m_jointList; b; b = b.m_next) {
			this._drawJoint(b)
		}
		for (b = world.m_bodyList; b; b = b.m_next) {
			for (a = b.GetShapeList(); a != null; a = a.GetNext()) {
				this._drawShape(a, b.GetMass(), b.IsSleeping())
			}
		}
	},
	SetDebugDraw: function(a) {
		a = a || {};
		this.ctx = a.ctx;
		this.ctx.width = a.width;
		this.ctx.height = a.height
	},
	_drawJoint: function(f) {
		var d = f.m_body1,
			b = f.m_body2,
			c = d.m_position,
			a = b.m_position,
		...
		switch (f.m_type) {
			...
			case b2Joint.e_pulleyJoint:
				...
				break;
			default:
				if (d == world.m_groundBody) {
					this.ctx.moveTo(g.x, g.y);
					this.ctx.lineTo(a.x, a.y)
				} else {
					if (b == world.m_groundBody) {
						this.ctx.moveTo(g.x, g.y);
	...
	_drawShape: function(j, l, f) {
		var q = "rgba(180,180,180,.75)",
			c = "rgba(180,180,180,.33)",
			g, o, p, m, b, h, e, n, k, a;
		if (!l) {
			q = "rgba(128,230,128,.75)";
			c = "rgba(128,230,128,.33)"
		} else {
			if (f) {
				q = "rgba(230,180,180,.75)";
				c = "rgba(230,180,180,.33)"
			}
		}
		this.ctx.strokeStyle = q;
		...
		switch (j.m_type) {
			case b2Shape.e_circleShape:
				...
				e = 0;
				n = 2 * Math.PI / h;
				this.ctx.moveTo(m.x + b, m.y);
				for (g = 0; g < h; g++) {
					k = new b2Vec2(b * Math.cos(e), b * Math.sin(e));
					o = b2Math.AddVV(m, k);
					this.ctx.lineTo(o.x, o.y);
					e += n
				}
				this.ctx.lineTo(m.x + b, m.y);
				this.ctx.moveTo(m.x, m.y);
				a = j.m_R.col1;
				m = new b2Vec2(m.x + b * a.x, m.y + b * a.y);
				this.ctx.lineTo(m.x, m.y);
				break;
			case b2Shape.e_polyShape:
				p = b2Math.AddVV(j.m_position, b2Math.b2MulMV(j.m_R, j.m_vertices[0]));
				this.ctx.moveTo(p.x, p.y);
				for (g = 0; g < j.m_vertexCount; g++) {
					o = b2Math.AddVV(j.m_position, b2Math.b2MulMV(j.m_R, j.m_vertices[g]));
					this.ctx.lineTo(o.x, o.y)
				}
...
b2Joint.Create = function(b, a) {
	var c = null;
	switch (b.type) {
		case b2Joint.e_distanceJoint:
			c = new b2DistanceJoint(b);
			break;
		case b2Joint.e_mouseJoint:
			c = new b2MouseJoint(b);
			break;
		case b2Joint.e_prismaticJoint:
			c = new b2PrismaticJoint(b);
			break;
		case b2Joint.e_revoluteJoint:
			c = new b2RevoluteJoint(b);
			break;
		case b2Joint.e_pulleyJoint:
			c = new b2PulleyJoint(b);
			break;
...
Object.extend(b2PulleyJointDef.prototype, {
	...
	maxLength2: null,
	ratio: null,
	Initialize: function(f, d, c, a, e, b, g) {
		this.body1 = f;
		this.body2 = d;
		this.groundPoint1 = c;
		this.groundPoint2 = a;
